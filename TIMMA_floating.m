...
% This version contains min-max average
% Tao Xu
function [M,err,k_set] = TIMMA_floating(k,drugs, protected_list, initial_list, S,spec, max_k,k_names,k_names0,ix2)
% function [M,err,k_set] = TIMMA_floating(k, drugs, protected_list, initial_list, S,spec, max_k,k_names,k_names0,ix2)
% [drugs,k] = size(S);
% tic();
...
%   fprintf(1,'Start kinase=%d\n',find(k_set));
    ...
    while(sum(k_set) <= max_k)
        ...
        [dummy,err0] = TIMMA(drugs, S_temp,y,sum(k_temp),LOO);
        ...
        for i = 1:k
            ...
            if(~k_set(i))
                ...
                err(i) = 0;
                [err1] = TIMMA_search(S(:,i),IM_ds,IM_is,IM_os,y,LOO);
        ...
        for i = 1:k
            ...
            if(k_set(i)) % if removing i
                ...
                S_temp = S(:,k_temp2);
                [dummy,err1] = TIMMA(drugs, S_temp,y,sum(k_temp),LOO);
...
%S_temp is made of the columns of included
S_temp = S(:,k_temp2);
[M,err] = TIMMA(drugs, S_temp,y,sum(k_temp),LOO);
% toc();
